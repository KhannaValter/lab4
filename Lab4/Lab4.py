...
def is_url_good(given_url): # проверка ccылки на не подходящие шасширения
    if (given_url is None or len(given_url) < 2 or given_url.find('#') != -1 or
            given_url.find('.mp3') != -1 or given_url.find('.jpg') != -1 or
            given_url.find('.php') != -1 or   given_url.find('javascript') != -1):
        return False
    else:
        return True
...
def make_valid_url_string(link):
    global main_pg
    if not is_url_good(link):
        return '0'
    else:
        if link.find('http:') == -1 or link.find('https:') == -1: # проверка что ссылка именно html
...
def get_ranks_dict(relationships_list):
    ...
    for i in range(100):
        for link in links:
            ...
            ranks[link] = 0.4 / len(links) + (1 - 0.4) * ranks_sum # 0.4 - d (коеф. затухання), расчет
            # расчет PageRank
    ranks_sum = sum(ranks.values())
    for page in ranks.keys():
        ranks[page] /= ranks_sum # нормализация PageRank
...
def run_op():
    ...
    links = []
    for line in soup.find_all('a'): # ищут тєг <a>
        if len(links) < 4:
            href = line.get('href') # ищут атрибут href - саму ссылку
            links.append([main_pg, href]) # добавление в двойной массик ссылки, и ссылок которые присутствуют на странице
        else:
            break
    links_number = 0
    for i in range(4):
    ...
    result_lbl.config(text=top_ten_links) # добавление 10 топ-ссылок
    d = dict(graph.degree)
    nx.draw(graph, with_labels=False, node_size=[d[k] * 60 for k in d], node_color="red", alpha=0.5, arrows=True,
            pos=nx.spring_layout(graph)) #добавление в граф
    plt.show()
    plt.savefig("output/lab4graph")


master = tkinter.Tk()
main_pg = ''
main_page_enter = tkinter.Entry(master, width=150)
main_page_enter.grid(row=0, column=0)
run_btn = tkinter.Button(master, text='Вычислить!', command=run_op)
